...
;; **Note:** there's an important property here that's worth calling out explicitly. Two columns which are `=` can
;; be returned by eg. `visible-columns` on two different queries. This is why it's important that the caches used where
;; `a-query` and `stage-number` are provided ([[display-info]]) are side channel caches that only cache for a specific
;; instance, not for any `=` value. The different queries might lead to different display names or other details.
;; The CLJS -> JS conversion doesn't know about queries, so it uses `=`-based LRU caches.
(declare ^:private display-info->js)

(defn- display-info-map->js* [x]
  (reduce (fn [obj [cljs-key cljs-val]]
            (set! js/window.__DIJS_recurses (inc js/window.__DIJS_recurses))
...
(defn ^:export display-info
  ...
  (lib.cache/side-channel-cache
    ...
    (fn [x] (display-info* a-query stage-number x))))
